...
function BigCalendar({ weddings, team, onEventClick }) {
  ...
  const dayMap=useMemo(()=>{const map={};weddings.forEach(w=>(w.eventDays||[]).forEach(ed=>{if(!map[ed.date])map[ed.date]=[];const crew=team.filter(m=>m.hires.some(h=>h.wedding===w.name&&h.date===ed.date));map[ed.date].push({weddingName:w.name,event:ed.event,crew,wedding:w,ed});}));return map;},[weddings,team]);
...
function MobileCalendar({ weddings, team }) {
  ...
  const events=useMemo(()=>{
    ...
    weddings.forEach(w=>(w.eventDays||[]).forEach(ed=>{
      ...
      if(ey===y&&em===mo+1){const crew=team.filter(m=>m.hires.some(h=>h.wedding===w.name&&h.date===ed.date));list.push({...ed,weddingName:w.name,crew,wedding:w});}
    }));
...
function TeamCalendar({ myHires, weddings, team, myName }) {
  ...
  const monthEvents=useMemo(()=>{
    ...
    weddings.forEach(w=>(w.eventDays||[]).forEach(ed=>{
      const [ey,em]=ed.date.split("-").map(Number);
      if(ey===y&&em===mo+1){const myHire=myHires.find(h=>h.wedding===w.name&&h.date===ed.date);const allCrew=team.filter(m=>m.hires.some(h=>h.wedding===w.name&&h.date===ed.date));list.push({date:ed.date,event:ed.event,startTime:ed.startTime,endTime:ed.endTime,weddingName:w.name,wedding:w,myHire,allCrew});}
    }));
    return list.sort((a,b)=>a.date.localeCompare(b.date));
      ...
      {open&&(
        <div style={{marginTop:12,paddingTop:12,borderTop:"1px solid #1e1a16"}}>
          ...
          :e.allCrew.map(member=>{
            const memberHire=member.hires.find(h=>h.wedding===e.weddingName&&h.date===e.date); const isMe=member.name===myName;
            return (
...
function TeamView({ team, weddings, adminProfile }) {
  ...
  async function handleDirectConfirm(hire, memberName) {
    const key=`${hire.wedding}|${hire.date}|${hire.event}`;
    setConfirmingId(key);
    // Send WA
          ...
          <div>
            {upcomingHires.length===0
              ...
              :upcomingHires.map((h,i)=>{
                ...
                const key=`${h.wedding}|${h.date}|${h.event}`;
...
function AdminApp({ user, onLogout }) {
  ...
  function saveWedding(){
    ...
    if(editWedding){
      ...
      setWeddings(weddings.map(w=>w.id===editWedding.id?{...w,...wForm}:w));
    }else{
      setWeddings([...weddings,{id:Date.now(),...wForm}]);
    }
  ...
  function addBulkHire(memberId){
    ...
    const newHires=hireForm.selectedEventDays.map(key=>{const[date,...evParts]=key.split("|");const event=evParts.join("|");return{wedding:hireForm.wedding,event,date,status:hireForm.status,dayType:hireForm.dayType,hireRole:hireForm.hireRole,paid:false};});
  ...
  // FEATURE 5: Dashboard filtered events
  ...
  const dashPages=Math.ceil(allDashEvents.length/DASH_PER_PAGE);
        ...
        {view==="dashboard"&&(
          <div className="fade-in">
            ...
            {dashEvents.map((e,i)=>{
              ...
              return (
                <div key={i} style={{background:"#0e0c0a",border:"1px solid #1e1a16",borderLeft:`3px solid ${evColor(e.event)}`,borderRadius:6,padding:"12px 14px",marginBottom:8,cursor:"pointer"}} onClick={()=>{setSelectedWedding(weddings.find(w=>w.name===e.weddingName));setView("wedding-detail");}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                    <div>
            ...
            <div>
              ...
              <div style={{maxHeight:520,overflowY:"auto",paddingRight:4}}>
                {dashEvents.map((e,i)=>{
                  ...
                  return (
                    <div key={i} style={{background:"#0e0c0a",border:"1px solid #1e1a16",borderLeft:`3px solid ${evColor(e.event)}`,borderRadius:4,padding:"12px 16px",marginBottom:8,display:"flex",justifyContent:"space-between",alignItems:"center",cursor:"pointer",opacity:isPast?0.6:1}} onClick={()=>{setSelectedWedding(weddings.find(w=>w.name===e.weddingName));setView("wedding-detail");}}>
                      <div>
                        <div style={{fontSize:15,fontWeight:500}}>{e.weddingName}</div>
                        <div style={{fontSize:12,color:evColor(e.event),fontFamily:"'DM Mono',monospace",marginTop:2}}>{e.event}</div>
                        <div style={{fontSize:11,color:"#5a5048",fontFamily:"'DM Mono',monospace",marginTop:2}}>{e.date}{e.startTime&&` · ⏰ ${e.startTime}`}</div>
                  ...
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                    ...
                    <div style={{display:"flex",gap:6,flexWrap:"wrap",justifyContent:"flex-end",maxWidth:240}}>
                      {crew.length===0?<span style={{fontSize:12,color:"#3a3028"}}>No crew hired</span>:crew.map(c=>{
                        const ch=c.hires.find(h=>h.wedding===w.name&&h.date===ed.date);
      ...
      {showAddHire&&(()=>{const m=team.find(x=>x.id===showAddHire);const eventDays=getWeddingEventDays(hireForm.wedding);return(
        <div className={mClass} onClick={()=>setShowAddHire(null)}><div className="modal" onClick={e=>e.stopPropagation()}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            {/* FEATURE 1: Optional team member to pre-assign */}
            <div>
              <p style={{fontSize:10,fontFamily:"'DM Mono',monospace",color:"#5a5048",textTransform:"uppercase",marginBottom:8}}>Assign Team (optional)</p>
